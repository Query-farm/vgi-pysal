...
#
# Single image that serves BOTH transports of the PySAL VGI worker:
#   docker run ... IMG            -> HTTP server on $PORT (default, Fly.io / local)
#   docker run -i ... IMG stdio   -> stdio worker DuckDB spawns on-host
# See docker-entrypoint.sh. Deps install from PyPI (vgi-python and the PySAL
# stack are all published; geopandas/shapely ship manylinux wheels, so there is
# no system GEOS/GDAL to build) — no vendored checkouts, no pin rewriting.
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-pysal
...
LABEL org.opencontainers.image.title="vgi-pysal" \
      org.opencontainers.image.description="PySAL as a VGI worker for DuckDB/SQL (stdio + HTTP)" \
      org.opencontainers.image.source="${SOURCE_URL}" \
      org.opencontainers.image.version="${VERSION}" \
      org.opencontainers.image.revision="${GIT_COMMIT}" \
      org.opencontainers.image.licenses="MIT" \
      farm.query.vgi.volumes='[{"path":"/data","name":"vgi_pysal_state","purpose":"state","shared":true}]'

ENV PYTHONUNBUFFERED=1 \
    PIP_NO_CACHE_DIR=1 \
    PIP_DISABLE_PIP_VERSION_CHECK=1 \
    # Both state mechanisms default under one /data volume (see the label above).
    PYSAL_MODELS_DIR=/data/models \
    ...
    VGI_PYSAL_GIT_COMMIT=${GIT_COMMIT} \
...
# version is read from vgi_pysal/__init__.py by hatchling, so the wheel and the
# advertised implementation_version match the release with no .git present.
COPY pyproject.toml README.md LICENSE ./
COPY vgi_pysal ./vgi_pysal
