...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the PySAL
...
#   VGI_PYSAL_WORKER  worker LOCATION the .test files attach (a stdio command
#                       such as the installed `vgi-pysal`, or an http:// URL)
# Optional:
#   STAGE               scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_PYSAL_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
export PYSAL_MODELS_DIR="${PYSAL_MODELS_DIR:-$STAGE/models}"
mkdir -p "$PYSAL_MODELS_DIR"
...
echo "Running suite (worker: $VGI_PYSAL_WORKER) ..."
