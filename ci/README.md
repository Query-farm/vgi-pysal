# CI: the PySAL worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the PySAL
...
1. **Install the worker** — `uv sync --frozen` into a venv. The `vgi-pysal`
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_PYSAL_WORKER` at the installed `vgi-pysal` command,
   isolates the model registry under a temp `PYSAL_MODELS_DIR`, warms the
...
VGI_PYSAL_WORKER="$PWD/.venv/bin/vgi-pysal" \
