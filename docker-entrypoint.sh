...
#
# Dispatch the single vgi-pysal image into one of its two transports:
...
#   /data/models  -> PYSAL_MODELS_DIR       (model registry)
...
mkdir -p "${PYSAL_MODELS_DIR:-/data/models}" \
         "$(dirname "${VGI_WORKER_SQLITE_PATH:-/data/state/vgi_storage.db}")"

case "${1:-http}" in
  http)
    shift 2>/dev/null || true
    exec vgi-pysal-http --host 0.0.0.0 --port "${PORT:-8000}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec vgi-pysal "$@"
